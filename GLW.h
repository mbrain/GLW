...
namespace glw // All OneLoneCoder stuff will now exist in the "glw" namespace
{
	struct Pixel
	{
		union
	...
	public:
		glw::rcode AddToPack(std::string sFile);

	public:
		glw::rcode SavePack(std::string sFile);
		glw::rcode LoadPack(std::string sFile);
		glw::rcode ClearPack();

	public:
		glw::ResourcePack::sEntry GetStreamBuffer(std::string sFile);
	...
	public:
		...
		Sprite(std::string sImageFile, glw::ResourcePack *pack);
		Sprite(int32_t w, int32_t h);
		~Sprite();

	public:
		glw::rcode LoadFromFile(std::string sImageFile, glw::ResourcePack *pack = nullptr);
		glw::rcode LoadFromPGESprFile(std::string sImageFile, glw::ResourcePack *pack = nullptr);
		glw::rcode SaveToPGESprFile(std::string sImageFile);
	...
	public:
		void SetSampleMode(glw::Sprite::Mode mode = glw::Sprite::Mode::NORMAL);
	...
	public:
		glw::rcode	Construct(uint32_t screen_w, uint32_t screen_h, uint32_t pixel_w, uint32_t pixel_h);
		glw::rcode	Start();
	...
	public: // Draw Routines
		...
		// glw::Pixel::NORMAL = No transparency
		// glw::Pixel::MASK   = Transparent if alpha is < 255
		// glw::Pixel::ALPHA  = Full transparency
		void SetPixelMode(Pixel::Mode m);
		Pixel::Mode GetPixelMode();
		// Use a custom blend function
		void SetPixelMode(std::function<glw::Pixel(const int x, const int y, const glw::Pixel& pSource, const glw::Pixel& pDest)> pixelMode);
		...
		virtual void Draw(int32_t x, int32_t y, Pixel p = glw::WHITE);
		// Draws a line from (x1,y1) to (x2,y2)
		void DrawLine(int32_t x1, int32_t y1, int32_t x2, int32_t y2, Pixel p = glw::WHITE);
		// Draws a circle located at (x,y) with radius
		void DrawCircle(int32_t x, int32_t y, int32_t radius, Pixel p = glw::WHITE);
		// Fills a circle located at (x,y) with radius
		void FillCircle(int32_t x, int32_t y, int32_t radius, Pixel p = glw::WHITE);
		// Draws a rectangle at (x,y) to (x+w,y+h)
		void DrawRect(int32_t x, int32_t y, int32_t w, int32_t h, Pixel p = glw::WHITE);
		// Fills a rectangle at (x,y) to (x+w,y+h)
		void FillRect(int32_t x, int32_t y, int32_t w, int32_t h, Pixel p = glw::WHITE);
		// Draws a triangle between points (x1,y1), (x2,y2) and (x3,y3)
		void DrawTriangle(int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t x3, int32_t y3, Pixel p = glw::WHITE);
		// Flat fills a triangle between points (x1,y1), (x2,y2) and (x3,y3)
		void FillTriangle(int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t x3, int32_t y3, Pixel p = glw::WHITE);
		...
		void DrawString(int32_t x, int32_t y, std::string sText, Pixel col = glw::WHITE, uint32_t scale = 1);
	...
	private: // Inner mysterious workings
		...
		std::function<glw::Pixel(const int x, const int y, const glw::Pixel&, const glw::Pixel&)> funcPixelMode;
		...
		void glw_UpdateMouse(int32_t x, int32_t y);
		bool glw_OpenGLCreate();
		void glw_ConstructFontSheet();

#ifdef _WIN32
		// Windows specific window handling
		HWND glw_hWnd = nullptr;
		HWND glw_WindowCreate();
		std::wstring wsAppName;
		static LRESULT CALLBACK glw_WindowEvent(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam);
#else
		// Non-Windows specific window handling
		Display*				glw_Display = nullptr;
		Window					glw_WindowRoot;
		Window					glw_Window;
		XVisualInfo*            glw_VisualInfo;
		Colormap                glw_ColourMap;
		XSetWindowAttributes    glw_SetWindowAttribs;
		Display*				glw_WindowCreate();
#endif
	...
	class PGEX
	{
		friend class glw::GLWrapper;
	protected:
		static GLWrapper* pge;
	};
...
/*
	...
	If the glwGLWrapper.h is called from several sources it can cause
	...
	Class1.h	- Includes glwGLWrapper.h, overrides glw::GLWrapper
	Class1.cpp	- #define OLC_PGE_APPLICATION #include "Class1.h"
	Class2.h	- Includes Class1.h, which includes glwGLWrapper.h
	Class2.cpp	- #define OLC_PGE_APPLICATION #include "Class2.h"
	main.cpp	- Includes Class1.h and Class2.h

	If all else fails, create a file called "glwGLWrapper.cpp" with the following 
	two lines. Then you can just #include "glwGLWrapper.h" as normal without worrying
	about defining things. Dont forget to include that cpp file as part of your build!

	#define OLC_PGE_APPLICATION
	#include "glwGLWrapper.h"

*/

#ifdef GLW_APPLICATION
#undef GLW_APPLICATION
...
namespace glw
{
	...
	Sprite::Sprite(std::string sImageFile, glw::ResourcePack *pack)
	{
	...
	glw::rcode Sprite::LoadFromPGESprFile(std::string sImageFile, glw::ResourcePack *pack)
	{
		if (pColData) delete[] pColData;
		...
		// These are essentially Memory Surfaces represented by glw::Sprite
	...
	glw::rcode Sprite::SaveToPGESprFile(std::string sImageFile)
	{
		if (pColData == nullptr) return glw::FAIL;
	...
	glw::rcode Sprite::LoadFromFile(std::string sImageFile, glw::ResourcePack *pack)
	{
#ifdef _WIN32
		// Use GDI+
	...
	void Sprite::SetSampleMode(glw::Sprite::Mode mode)
	{
		modeSample = mode;
	}
	...
	{
		if (modeSample == glw::Sprite::Mode::NORMAL)
		{
	...
	Pixel Sprite::SampleBL(float u, float v)
	{
		...
		glw::Pixel p1 = GetPixel(x, y);
		glw::Pixel p2 = GetPixel(x+1, y);
		glw::Pixel p3 = GetPixel(x, y+1);
		glw::Pixel p4 = GetPixel(x+1, y+1);
	...
	glw::rcode ResourcePack::AddToPack(std::string sFile)
	{
		std::ifstream ifs(sFile, std::ifstream::binary);
		if (!ifs.is_open()) return glw::FAIL;
	...
	glw::rcode ResourcePack::SavePack(std::string sFile)
	{
		std::ofstream ofs(sFile, std::ofstream::binary);
		if (!ofs.is_open()) return glw::FAIL;
	...
	glw::rcode ResourcePack::LoadPack(std::string sFile)
	{
		std::ifstream ifs(sFile, std::ifstream::binary);
		if (!ifs.is_open()) return glw::FAIL;
	...
	glw::ResourcePack::sEntry ResourcePack::GetStreamBuffer(std::string sFile)
	{
		return mapFiles[sFile];
	}

	glw::rcode ResourcePack::ClearPack()
	{
		for (auto &e : mapFiles)
		{
	...
	glw::rcode GLWrapper::Construct(uint32_t screen_w, uint32_t screen_h, uint32_t pixel_w, uint32_t pixel_h)
	{
	...
	glw::rcode GLWrapper::Start()
	{
		...
		// Start the thread
		bAtomActive = true;
		std::thread t = std::thread(&GLWrapper::EngineThread, this);
	...
	void GLWrapper::SetDrawTarget(Sprite *target)
	{
		if (target)
			pDrawTarget = target;
		else
	...
	Sprite* GLWrapper::GetDrawTarget()
	{
		return pDrawTarget;
	}

	int32_t GLWrapper::GetDrawTargetWidth()
	{
		if (pDrawTarget)
	...
	int32_t GLWrapper::GetDrawTargetHeight()
	{
		if (pDrawTarget)
	...
	bool GLWrapper::IsFocused()
	{
		return bHasInputFocus;
	}

	HWButton GLWrapper::GetKey(Key k)
	{
		return pKeyboardState[k];
	}

	HWButton GLWrapper::GetMouse(uint32_t b)
	{
		return pMouseState[b];
	}

	int32_t GLWrapper::GetMouseX()
	{
		return nMousePosX;
	}

	int32_t GLWrapper::GetMouseY()
	{
		return nMousePosY;
	}

	int32_t GLWrapper::ScreenWidth()
	{
		return nScreenWidth;
	}

	int32_t GLWrapper::ScreenHeight()
	{
		return nScreenHeight;
	}

	void GLWrapper::Draw(int32_t x, int32_t y, Pixel p)
	{
	...
	void GLWrapper::SetSubPixelOffset(float ox, float oy)
	{
		fSubPixelOffsetX = ox * fPixelX;
		fSubPixelOffsetY = oy * fPixelY;
	}

	void GLWrapper::DrawLine(int32_t x1, int32_t y1, int32_t x2, int32_t y2, Pixel p)
	{
	...
	void GLWrapper::DrawCircle(int32_t x, int32_t y, int32_t radius, Pixel p)
	{
		int x0 = 0;
		int y0 = radius;
		int d = 3 - 2 * radius;
	...
	void GLWrapper::FillCircle(int32_t x, int32_t y, int32_t radius, Pixel p)
	{
	...
	void GLWrapper::DrawRect(int32_t x, int32_t y, int32_t w, int32_t h, Pixel p)
	{
	...
	void GLWrapper::Clear(Pixel p)
	{
		...
		for (int i = 0; i < pixels; i++)
			m[i] = p;
#ifdef OLC_DBG_OVERDRAW
		glw::Sprite::nOverdrawCount += pixels;
#endif
	}

	void GLWrapper::FillRect(int32_t x, int32_t y, int32_t w, int32_t h, Pixel p)
	{			
		int32_t x2 = x + w;
		int32_t y2 = y + h;
	...
	void GLWrapper::DrawTriangle(int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t x3, int32_t y3, Pixel p)
	{
	...
	void GLWrapper::FillTriangle(int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t x3, int32_t y3, Pixel p)
	{
	...
	void GLWrapper::DrawSprite(int32_t x, int32_t y, Sprite *sprite, uint32_t scale)
	{
		if (sprite == nullptr)
			return;
	...
	void GLWrapper::DrawPartialSprite(int32_t x, int32_t y, Sprite *sprite, int32_t ox, int32_t oy, int32_t w, int32_t h, uint32_t scale)
	{
		if (sprite == nullptr)
			return;
	...
	void GLWrapper::DrawString(int32_t x, int32_t y, std::string sText, Pixel col, uint32_t scale)
	{
		int32_t sx = 0;
		int32_t sy = 0;
	...
	void GLWrapper::SetPixelMode(Pixel::Mode m)
	{
		nPixelMode = m;
	}

	Pixel::Mode GLWrapper::GetPixelMode()
	{
		return nPixelMode;
	}

	void GLWrapper::SetPixelMode(std::function<glw::Pixel(const int x, const int y, const glw::Pixel&, const glw::Pixel&)> pixelMode)
	{
		funcPixelMode = pixelMode;
		nPixelMode = Pixel::Mode::CUSTOM;
	}

	void GLWrapper::SetPixelBlend(float fBlend)
	{
		fBlendFactor = fBlend;
	...
	bool GLWrapper::OnUserCreate()
	{ return false; }
	bool GLWrapper::OnUserUpdate(float fElapsedTime)
	{ return false; }
	bool GLWrapper::OnUserDestroy()
	{ return true; }
	//////////////////////////////////////////////////////////////////

	void GLWrapper::glw_UpdateMouse(int32_t x, int32_t y)
	{
	...
	void GLWrapper::EngineThread()
	{
		...
		while (bAtomActive)
		{
			...
			while (bAtomActive)
			{
				...
				XEvent xev;
				while (XPending(glw_Display))
				{
					XNextEvent(glw_Display, &xev);
					if (xev.type == Expose)
					{
						XWindowAttributes gwa;
						XGetWindowAttributes(glw_Display, glw_Window, &gwa);
					...
					{
						glw_UpdateMouse(xev.xmotion.x, xev.xmotion.y);
					}
...
#else
				glXSwapBuffers(glw_Display, glw_Window);
#endif
				...
				if (fFrameTimer >= 1.0f)
				{
					...
					std::string sTitle = "GE - " + sAppName + " - FPS: " + std::to_string(nFrameCount);
#ifdef _WIN32
#ifdef UNICODE
					SetWindowText(glw_hWnd, ConvertS2W(sTitle).c_str());
#else
					SetWindowText(glw_hWnd, sTitle.c_str());
#endif
#else					
					XStoreName(glw_Display, glw_Window, sTitle.c_str());
#endif
					nFrameCount = 0;
				}
			}
		...
		PostMessage(glw_hWnd, WM_DESTROY, 0, 0);
#else
		glXMakeCurrent(glw_Display, None, NULL);
		glXDestroyContext(glw_Display, glDeviceContext);
		XDestroyWindow(glw_Display, glw_Window);
		XCloseDisplay(glw_Display);
#endif

	}


	void GLWrapper::glw_ConstructFontSheet()
	{
		std::string data;
		...
		fontSprite = new glw::Sprite(128, 48);
		int px = 0, py = 0;
		for (int b = 0; b < 1024; b += 4)
		{
			...
			{
				int k = r & (1 << i) ? 255 : 0;
				fontSprite->SetPixel(px, py, glw::Pixel(k, k, k, k));
				if (++py == 48) { px++; py = 0; }
			}
		}
	}
...
#ifdef _WIN32
	HWND GLWrapper::glw_WindowCreate()
	{
		WNDCLASS wc;
		...
		wc.lpfnWndProc = glw_WindowEvent;
		wc.cbClsExtra = 0;
		wc.cbWndExtra = 0;
...
#ifdef UNICODE
		glw_hWnd = CreateWindowEx(dwExStyle, L"OLC_PIXEL_GAME_ENGINE", L"", dwStyle,
			30, 30, width, height, NULL, NULL, GetModuleHandle(nullptr), this);
#else
		glw_hWnd = CreateWindowEx(dwExStyle, "OLC_PIXEL_GAME_ENGINE", "", dwStyle,
			30, 30, width, height, NULL, NULL, GetModuleHandle(nullptr), this);
#endif
	...
	bool GLWrapper::glw_OpenGLCreate()
	{
		// Create Device Context
		glDeviceContext = GetDC(glw_hWnd);
	...
	LRESULT CALLBACK GLWrapper::glw_WindowEvent(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		static GLWrapper *sge;
		switch (uMsg)
		{
		case WM_CREATE:		sge = (GLWrapper*)((LPCREATESTRUCT)lParam)->lpCreateParams;	return 0;
		case WM_MOUSEMOVE:	
		{
...
#else
	// Do the Linux stuff!
	Display* GLWrapper::glw_WindowCreate()
	{
		XInitThreads();

		// Grab the deafult display and window
		glw_Display		= XOpenDisplay(NULL);
		glw_WindowRoot	= DefaultRootWindow(glw_Display);

		// Based on the display capabilities, configure the appearance of the window
		GLint glw_GLAttribs[] = { GLX_RGBA, GLX_DEPTH_SIZE, 24, GLX_DOUBLEBUFFER, None };
		glw_VisualInfo	= glXChooseVisual(glw_Display, 0, glw_GLAttribs);
		glw_ColourMap	= XCreateColormap(glw_Display, glw_WindowRoot, glw_VisualInfo->visual, AllocNone);
		glw_SetWindowAttribs.colormap = glw_ColourMap;

		// Register which events we are interested in receiving
		glw_SetWindowAttribs.event_mask = ExposureMask | KeyPressMask | KeyReleaseMask | ButtonPressMask | ButtonReleaseMask | PointerMotionMask | FocusChangeMask;

		// Create the window
		glw_Window		= XCreateWindow(glw_Display, glw_WindowRoot, 30, 30, nScreenWidth * nPixelWidth, nScreenHeight * nPixelHeight, 0, glw_VisualInfo->depth, InputOutput, glw_VisualInfo->visual, CWColormap | CWEventMask, &glw_SetWindowAttribs);

		Atom wmDelete = XInternAtom(glw_Display, "WM_DELETE_WINDOW", true);
		XSetWMProtocols(glw_Display, glw_Window, &wmDelete, 1);

		XMapWindow(glw_Display, glw_Window);
		XStoreName(glw_Display, glw_Window, "OneLoneCoder.com - Pixel Game Engine");
	...
	bool GLWrapper::glw_OpenGLCreate()
	{
		glDeviceContext = glXCreateContext(glw_Display, glw_VisualInfo, nullptr, GL_TRUE);
		glXMakeCurrent(glw_Display, glw_Window, glDeviceContext);
		...
		if (glSwapIntervalEXT)
			glSwapIntervalEXT(glw_Display, glw_Window, 0);
		else
		{
	...
	std::atomic<bool> GLWrapper::bAtomActive{ false };
	std::map<uint16_t, uint8_t> GLWrapper::mapKeys;
	glw::GLWrapper* glw::PGEX::pge = nullptr;
#ifdef OLC_DBG_OVERDRAW
	int glw::Sprite::nOverdrawCount = 0;
#endif
	//=============================================================
}
